(function() {
    ...
    function NavCtrl($state, artSrv,$uibModal) {
        ...
        function loadMedia(source) {
            ...
            if (source == 'REDDIT') {
                ctrl.artSrv.mediaSelect = 'REDDIT';
                ctrl.artSrv.getMedia().then(function(res) {
                    ctrl.artSrv.skipMediaCount = 30;
                });
            } else {
                ctrl.artSrv.mediaSelect = 'IG';
                ctrl.artSrv.getSocial().then(function(res) {
                    ctrl.artSrv.skipMediaCount = 30;
                });
            }
        }
        ...
        (function() {
            ...
            function init() {
                ctrl.menu = document.getElementById('bt-menu');
                // var trigger = menu.querySelector('a.bt-menu-trigger'),
                // event type (if mobile use touch events)
                eventtype = mobilecheck() ? 'touchstart' : 'click',

                resetMenu = function() {
                    classie.remove(ctrl.menu, 'bt-menu-open');
                    classie.add(ctrl.menu, 'bt-menu-close');
                },
                closeClickFn = function(ev) {
                    console.log('closeclick hit');
                    resetMenu();
                ...
                ctrl.menu.appendChild(overlay);

                ctrl.trigger = function() {


                    console.log('triggered menu');
                    if (classie.has(ctrl.menu, 'bt-menu-open')) {
                        console.log('classie has');
                        resetMenu();
                    } else {
                        classie.remove(ctrl.menu, 'bt-menu-close');

                        classie.add(ctrl.menu, 'bt-menu-open');
